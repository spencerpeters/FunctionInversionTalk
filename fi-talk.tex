...
%    % \pause
 ...
 \item No longer possible to cover the entire range.
   ...
   \pause
 \item But, one can cover a small fraction with disjoint paths, then compose with a ``random'' function and repeat. \inlineauthor{hellman}
   % $T \lessim N^2 / S^2$.
   \pause
 \item For \emph{random} functions, Hellman showed (heuristically) this can be made to work.
 % \pause
 ...
 \begin{center}
   ...
   \node[roundnode, draw=blue!60, fill=orange!5] at (bB) {};
   \node[roundnode, draw=blue!60, fill=orange!5] at (bC) {};
    \draw  ($(bC) + (-2, 1)$) node[roundnode] (X) {} edge[-Latex, dashed] (bC);
    \draw  ($(bC) + (-1, 1)$) node[roundnode] (X) {} edge[-Latex, dashed] (bC);
    ...
    \only<5->{
   \begin{scope}[rotate around = {45:(3,0)}]
