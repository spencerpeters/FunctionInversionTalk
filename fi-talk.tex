...
\begin{frame}
  \frametitle{Function Inversion}
  \begin{itemize}
  \item Given a function\only<2->{, and a point $y$ in its range,}\only<3->{ \newline find $x$ with $f(x) = y$.}
    \vspace{0.2em}
  \end{itemize}
  \begin{tikzpicture}
    \only<.-2>{\node (A) at (0, 0) {$\{1, 2, \dots, N\} =: [N]$};}
...
\end{frame}
  ...
  \begin{frame}
    ...
    \begin{itemize}
    \item If $f$ is a permutation, \newline its \emph{graph} is a disjoint union of cycles.
    \end{itemize}
    \vspace{1ex}
    \onslide<+->
    % LATER animate these!
...
% \pause
 ...
 \begin{frame}
   ...
   \begin{itemize}
   \item The NSA wants to break cryptography based on a widely used cryptographic function, such as AES-128.
     \pause
 ...
 % \pause
  ...
  \item A: Sort of and sort of!
    % In this talk:
  % \begin{itemize}
  %   \item Result 1: A simple improvement to Fiat and Naor's algorithm when $T > S$.
  %   \item Result 2: A tight lower bound for a natural class of function inversion algorithms.
  %   \end{itemize}
  \end{itemize}
  }
...
%   \item Q: Can we improve Fiat-Naor? Can we improve Yao's lower bound?
%     \pause
%   \item A: Sort of and sort of! In this talk:
%      \scalebox{0.85}{\begin{minipage}{1.2\textwidth}
%   \begin{itemize}
%     \item Result 1: A simple improvement to Fiat and Naor's algorithm when $T > S$.
%     \item Result 2: A tight lower bound for a natural class of function inversion algorithms.
...
%   \end{itemize}
  ...
  \begin{frame}
    ...
    \item Not in this talk: equivalences between variants of function inversion.
        ...
        %   \item
        %     A larger codomain does not make the problem harder (us and \cite{CorriganGibbs19}) \inlineauthor{cg} \inlineauthor{kogan}
...
\begin{axis}[
    ...
    xlabel = {\(\log(S)\textcolor{gray}{/\log(N)}\)},
    ylabel = {\(\log(T)\textcolor{gray}{/\log(N)}\)},
    legend pos=south east, % position the legend in the north-west corner
...
%     % \frametitle{Beyond permutations: the gap}
     \frametitle{Background: beyond permutations}
...
%    % \pause
 ...
 % \item But \emph{arbitrary} functions $f$ may have points with very many inverses (``junction points''), which cause paths to overlap very quickly.
  ...
  \begin{frame}
    \frametitle{Background: Fiat-Naor}
 \begin{itemize}
...
% \end{frame}
  ...
  \begin{frame}
    \frametitle{Result 2:  Background}
    \begin{itemize}
  ...
  \begin{frame}
    ...
    \item This implicitly defines $T$ candidate inverses for the online algorithm to check, achieving the tradeoff $S = O(N \log (N / T))$.
      % just store all but $\log(T)$ bits of one of its inverses (implicitly defining $T$ candidate inverses). This achieves $S = O(N \log (N / T))$.
    % \item It achieves the  $S = O(N \log (N / T))$.
    ...
    \item
      ...
      % We show that \textbf{all \emph{guess-and-check} algorithms satisfy the matching lower bound $S = \Omega(N \log (N/T))$}.
    \end{itemize}
  \end{frame}
